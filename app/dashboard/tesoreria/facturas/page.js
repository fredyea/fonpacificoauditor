...
export default function Compromisos() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'maestrocuentasxcobrar');
  ...
  const handleConfirm = async () => {
    try {
      ...
      formdata.append('id_cuentaxcobrar', iddocumento);

      const res = await fetch(_servidorapi + 'habilitarcxc', {
        method: 'POST',
        body: formdata,
      });
      setShowModal(false);
      // Recargar los datos después de procesar
      toast.success('Factura habilitada correctamente');
      const response = await fetch(_servidorapi+'maestrocuentasxcobrar');
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
  ...
  return (
    ...
    <button
      onClick={() => handleProcess(item.id_cuentaxcobrar)}
                    ...
                    <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-gray-800/95 p-6 text-left align-middle shadow-xl transition-all">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-300">
                          ¿Está seguro que desea habilitar la Factura?
                        </p>
                      </div>
