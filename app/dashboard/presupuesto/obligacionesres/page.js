...
export default function Compromisos() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'maestroobligacionesreservas');
  ...
  const handleConfirm = async () => {
    try {
      ...
      formdata.append('id_obligacion', iddocumento);

      const res = await fetch(_servidorapi + 'habilitarobligacionreservas', {
        method: 'POST',
        body: formdata,
      });
      setShowModal(false);
      // Recargar los datos después de procesar
      toast.success('Obligacion habilitada correctamente');
      const response = await fetch(_servidorapi+'maestroobligacionesreservas');
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
  ...
  return (
    <div className="w-screen max-w-none py-2" style={{ marginLeft: 'calc(50% - 50vw)', marginRight: 'calc(50% - 50vw)', paddingLeft: '16px', paddingRight: '16px' }}>
      <ContentLayout title="Obligaciones Para pago de Reservas">
    ...
    <button
      onClick={() => handleProcess(item.id_obligacion)}
                    ...
                    <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-gray-800/95 p-6 text-left align-middle shadow-xl transition-all">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-300">
                          ¿Está seguro que desea la obligación?
                        </p>
                      </div>
