...
export default function Compromisos() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'maestroobligacionesdp');
  ...
  const handleConfirm = async () => {
    try {
      ...
      const res = await fetch(_servidorapi + 'habilitarobligaciondp', {
        method: 'POST',
        body: formdata,
      });
      setShowModal(false);
  ...
  return (
    <div className="w-screen max-w-none py-2" style={{ marginLeft: 'calc(50% - 50vw)', marginRight: 'calc(50% - 50vw)', paddingLeft: '16px', paddingRight: '16px' }}>
      <ContentLayout title="Obligaciones De Depositos Provisionales">
