...
export default function Adiciones() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'maestro_ppp_rebajas');
  ...
  const handleConfirm = async () => {
    try {
      ...
      formdata.append('id_rebaja', iddocumento);

      const res = await fetch(_servidorapi + 'habilitarrebaja', {
        method: 'POST',
        body: formdata,
      });
      setShowModal(false);
      ...
      console.error('Error al procesar la rebaja:', error);
    }
  };

  return (
    <ContentLayout title="Reabajas Presupuestales">
                  ...
                  <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-gray-800/95 p-6 text-left align-middle shadow-xl transition-all">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-300">
                        ¿Está seguro que desea habilitar esta rebaja?
                      </p>
                    </div>
