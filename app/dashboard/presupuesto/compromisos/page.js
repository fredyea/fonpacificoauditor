...
export default function Compromisos() {
  ...
  const handleConfirm = async () => {
    try {
      ...
      console.log('Procesando compromiso:', selectedId);
      const formdata = new FormData();
      formdata.append('id_registro', iddocumento);

      const res = await fetch(_servidorapi + 'habilitarcompromiso', {
        method: 'POST',
        body: formdata,
      });
      setShowModal(false);
      // Recargar los datos después de procesar
      toast.success('Compomiso habilitado correctamente');
      const response = await fetch(_servidorapi+'maestrocompromisos');
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
  ...
  return (
    <div className="w-screen max-w-none py-2" style={{ marginLeft: 'calc(50% - 50vw)', marginRight: 'calc(50% - 50vw)', paddingLeft: '16px', paddingRight: '16px' }}>
      <ContentLayout title="Compromisos Presupuestales">
                    ...
                    <tbody className="divide-y divide-gray-700 bg-gray-900">
                      {currentData.map((item, index) => (
                        <tr 
                          ...
                          className={`
                            ${item.saldo < 0  ? 'bg-red-900/50 hover:bg-red-800/50' : 'hover:bg-gray-800'}
                            transition-colors
                          `}
                        >
    ...
    <button
      onClick={() => handleProcess(item.id_registro)}
                    ...
                    <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-gray-800/95 p-6 text-left align-middle shadow-xl transition-all">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-300">
                          ¿Está seguro que desea habilitar el compromiso?
                        </p>
                      </div>
