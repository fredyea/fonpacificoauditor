...
export default function Compromisos() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'maestrovales');
  ...
  const handleConfirm = async () => {
    try {
      ...
      formdata.append('id_vale', iddocumento);

      const res = await fetch(_servidorapi + 'habilitarvale', {
        method: 'POST',
        body: formdata,
      });
      setShowModal(false);
      // Recargar los datos después de procesar
      toast.success('Vale habilitado correctamente');
      const response = await fetch(_servidorapi+'maestrovales');
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
  ...
  return (
    <div className="w-screen max-w-none py-2" style={{ marginLeft: 'calc(50% - 50vw)', marginRight: 'calc(50% - 50vw)', paddingLeft: '16px', paddingRight: '16px' }}>
      <ContentLayout title="Egresos sin Obligacion (Vales)">
    ...
    <button
      onClick={() => handleProcess(item.id_vale)}
                    ...
                    <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-gray-800/95 p-6 text-left align-middle shadow-xl transition-all">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-300">
                          ¿Está seguro que desea habilitar el vale?
                        </p>
                      </div>
