...
export default function Compromisos() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'maestrorecaudos');
  ...
  const handleProcess = (id_doc) => {
    setIddocumento(id_doc)
    setShowModal(true);
  };
  ...
  const handleConfirm = async () => {
    try {
      ...
      console.log('Procesando Obligaciones:', selectedId);
      const formdata = new FormData();
      formdata.append('id_recaudo', iddocumento);

      const res = await fetch(_servidorapi + 'habilitarrecaudo', {
        method: 'POST',
        body: formdata,
      });
      setShowModal(false);
      // Recargar los datos después de procesar
      toast.success('Recaudo habilitado correctamente');
      const response = await fetch(_servidorapi+'maestrorecaudos');
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
      console.error('Error al procesar la el recaudo:', error);
    }
  };
  ...
  return (
                  ...
                  <table className="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody className="divide-y divide-gray-700 bg-gray-900">
                      {currentData.map((item, index) => (
                        <tr 
                          key={item.id_cuentaxcobrar}
                          className={`
                            ${item.valor < 0  ? 'bg-red-900/50 hover:bg-red-800/50' : 'hover:bg-gray-800'}
                            transition-colors
                          `}
                        >
    ...
    <button
      onClick={() => handleProcess(item.id_recaudo)}
                    ...
                    <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-gray-800/95 p-6 text-left align-middle shadow-xl transition-all">
                      ...
                      <div className="mt-2">
                        <p className="text-sm text-gray-300">
                          ¿Está seguro que desea habilitar el recaudo?
                        </p>
                      </div>
