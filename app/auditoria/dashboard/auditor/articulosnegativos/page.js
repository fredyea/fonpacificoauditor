...
export default function Compromisos() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'articulosnegativos');
  ...
  return (
    <div className="w-screen max-w-none py-2" style={{ marginLeft: 'calc(50% - 50vw)', marginRight: 'calc(50% - 50vw)', paddingLeft: '16px', paddingRight: '16px' }}>
      <ContentLayout title="Articulos Negativos">
                ...
                <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg">
                  <table className="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody className="divide-y divide-gray-700 bg-gray-900">
                      {loading ? (
                        <tr>
                          <td colSpan={9} className="text-center py-8 text-gray-400 text-lg">
                            Cargando...
                          </td>
                        </tr>
                          ...
                          >
                            ...
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-300 text-right">
                              {new Intl.NumberFormat("es-CO", {
                                ...
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              }).format(item.adiciones_articulo)}
                            </td>
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-300 text-right">
                              {new Intl.NumberFormat("es-CO", {
                                style: "decimal",
                            ...
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-300 text-right">
                              {new Intl.NumberFormat("es-CO", {
                                ...
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              }).format(item.traslados_articulo)}
                            </td>
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-300 text-right">
                              {new Intl.NumberFormat("es-CO", {
                                style: "decimal",
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              }).format(item.inicial_articulo+item.adiciones_articulo-item.rebajas_articulo+item.traslados_articulo)}
                            </td>
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-300 text-right">
                              {new Intl.NumberFormat("es-CO", {
                                style: "decimal",
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              }).format(item.disponibilidades_articulo)}
                            </td>
                            <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-300 text-right">
                              {new Intl.NumberFormat("es-CO", {
                                style: "decimal",
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              }).format((item.inicial_articulo+item.adiciones_articulo-item.rebajas_articulo+item.traslados_articulo)-item.disponibilidades_articulo)}
                            </td>
                          </tr>
                        ))
                      ) : (
