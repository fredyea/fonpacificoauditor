...
export default function Adiciones() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'maestro_ppp_adiciones');
  ...
  const handleConfirm = async () => {
    try {
      ...
      formdata.append('id_adicion', iddocumento);

      const res = await fetch(_servidorapi + 'habilitaradicion', {
        method: 'POST',
        body: formdata,
      });
      ...
      try {
        await sendNotification(1, 'Nueva Adición Habilitada', `La adición #${iddocumento} ha sido habilitada`);
  ...
  return (
    <ContentLayout title="Adiciones Presupuestales">
  ...
  <button
    onClick={() => handleProcess(item.id_adicion)}
                  ...
                  <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-gray-800/95 p-6 text-left align-middle shadow-xl transition-all">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-300">
                        ¿Está seguro que desea habilitar esta Adición?
                      </p>
                    </div>
