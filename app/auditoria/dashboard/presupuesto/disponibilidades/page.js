...
export default function Disponibilidades() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(_servidorapi+'maestrodisponibilidades');
  ...
  const handleConfirm = async () => {
    try {
      ...
      console.log('Procesando Disponibilidad:', selectedId);
      const formdata = new FormData();
      formdata.append('id_disponibilidad', iddocumento);

      const res = await fetch(_servidorapi + 'habilitardisponibilidad', {
        method: 'POST',
        body: formdata,
      });
      setShowModal(false);
      // Recargar los datos después de procesar
      toast.success('Disponibilidad habilitada correctamente');
      const response = await fetch(_servidorapi+'maestrodisponibilidades');
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
  ...
  return (
    <ContentLayout title="Disponibilidades Presupuestales">
  ...
  <button
    onClick={() => handleProcess(item.id_disponibilidad)}
                  ...
                  <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-gray-800/95 p-6 text-left align-middle shadow-xl transition-all">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-300">
                        ¿Está seguro que desea habilitar esta Disponibilidad?
                      </p>
                    </div>
